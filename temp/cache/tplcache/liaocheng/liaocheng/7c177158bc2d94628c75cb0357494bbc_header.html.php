<?php if(!defined('IN_TXTCMS')){define('IN_TXTCMS',true);} ?><div id="header">
    <div id="top">
        ...
        <div id="top_searchs">
            ...
            <div id="top_login">欢迎您：网友，请
                ...
                <a href="/Api/qq/"
                   style="background:url(<?php echo $this->_var['theme_path']; ?>/img/qq_ico.gif) no-repeat 0px 0px;padding:2px 0 3px 18px;/" title="用QQ账号登录">QQ登录</a>
    ...
    <ul id="navB">
	    <?php $item = TypeName('company', 23)?>
	    <?php for ($i=1; $i< count($item); $i++):?>
	    <?php if ($i == rand(1,10) || $i == rand(1,10)):?>
        <li><a href="<?php echo $this->tag_function_geturl(array( 'tpl'=>"", 'fan'=>"", )); ?>" title="<?=$item[$i][0]?>"><?=$item[$i][0]?><img src="<?php echo $this->_var['theme_path']; ?>/img/jian.gif"></a></li>
	    <?php else:?>
        <li><a href="<?php echo $this->tag_function_geturl(array( 'tpl'=>"", 'fan'=>"", )); ?>" title="<?=$item[$i][0]?>"><?=$item[$i][0]?></a></li>
        <?php endif; ?>
		<?php endfor;?>
    </ul>
</div>