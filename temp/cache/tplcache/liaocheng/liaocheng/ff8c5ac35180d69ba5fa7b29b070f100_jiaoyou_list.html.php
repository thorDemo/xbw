...
<div class="content">
	<div class="l">
		<div class="xlb">
            <?php $item = typeName('news', 20)?>
            <?php foreach ($item as $it):?>
				<a href="<?=randList('jiaoyou')?>"><?=$it['name']?><span class="js">(<?php echo rand(100,500)?>)</span></a>
            <?php endforeach;?>
		</div>
		<div class="dq">
			<b>按地区：</b>
            ...
            <?php foreach ($place as $item):?>
				<a href="<?=randList('jiaoyou')?>"><?=$item?></a>
            <?php endforeach;?>
		</div>
		<div class="list_dh">
			<div class="list_dh_l">全部信息</div>
			<div class="list_dh_c">位于：<a href="/">聊城信息港</a> &raquo; <a href="<?=randList('jiaoyou')?>">二手市场</a></div>
			<div class="list_dh_r"><a href="<?=randList('jiaoyou')?>" target="_blank">？信息如何置顶</a></div>
		</div>
		<div class="list" id="keylist">
            <?php $article = randArticle('jiaoyou', 34)?>
			<dl>
		...
		<div class="page">
			...
			<a href='<?=randList('jiaoyou')?>'>上一页&gt;</a>
            <?php foreach (range(1,10) as $item):?>
				<a href='<?=randList('jiaoyou')?>'><?=$item?></a>
            <?php endforeach;?>
			<a href="<?=randList('jiaoyou')?>">下一页&gt;</a>
		</div>
		<div class="page_fa">
			<span class="right">
				<a href="#top">回到顶部↑</a>
			</span>未找到想要的信息？发布一条信息，让信息主动来找您
			<a href="<?=randList('jiaoyou')?>">？我要发布分类信息</a>
		</div>
	</div>
	<div class="r">
		...
		<div class="k208 left"><a href="<?=randList('jiaoyou')?>"><img src="<?=randPic('face')?>" alt="发布信息" border="0"/></a></div>
	</div>
</div>
